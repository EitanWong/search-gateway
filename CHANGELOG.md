...
- Replace the Cloudflare one-click path with true zero-config deployment: isolated `deploy-template/` subdirectory, `wrangler.jsonc`, Cloudflare template metadata, build/deploy command prefills, open-by-default auth mode, and optional bearer-token hardening.
