...
function extractPageMeta(html, finalUrl = "") {
  ...
  return {
    title: attr(/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']/i)
    ...
    published_at: normalizeDate(attr(/<meta[^>]+property=["']article:published_time["'][^>]+content=["']([^"']+)["']/i)
      || attr(/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']article:published_time["']/i)
      || attr(/<time[^>]+datetime=["']([^"']+)["']/i)),
    author: attr(/<meta[^>]+name=["']author["'][^>]+content=["']([^"']*)["']/i)
    ...
    modified_at: normalizeDate(attr(/<meta[^>]+property=["']article:modified_time["'][^>]+content=["']([^"']+)["']/i)
...
function htmlToMarkdown(html, baseUrl = "") {
  const src = String(html || "").slice(0, 512 * 1024)
...
function detectDynamicPage(html, text) {
  ...
  const isDynamic = emptySpaRoot || sparseLargePage || (frameworkMarker && lowTextYield);
  return isDynamic ? {
    is_dynamic: true,
...
async function readJson(request) {
  ...
  if (contentLength > MAX_JSON_BODY_CHARS) {
    throw new Error(`request body too large; max ${MAX_JSON_BODY_CHARS} bytes`);
  }
  const text = await readTextLimited(new Response(request.body, { headers: request.headers }), MAX_JSON_BODY_CHARS);
  try {
...
async function checkRateLimit(request, env, path) {
  ...
  if (current >= limit) {
    return json({ ok: false, error: "rate limit exceeded", retry_after_seconds: 60 }, 429, { "retry-after": "60" });
  }
...
function queryTokens(query) {
  return [...new Set(String(query).toLowerCase().match(/[a-z0-9]+/g) || [])].filter((token) => token.length > 1);
}
...
function resultScore(result, tokens) {
  return tokenHits(result.title, tokens) * 10
    + tokenHits(result.snippet, tokens) * 2
    + Math.max(0, result.providers.length - 1) * 8;
}
...
function mergeRankResults(results, query, limit) {
...
async function runSearch(body, env) {
  ...
  if (strategy === "aggregate") {
    ...
    const results = mergeRankResults(settled.flatMap((item) => item.results), query, limit);
    return {
      ok: results.length > 0,
  ...
  for (const provider of providerOrder) {
    try {
      const results = mergeRankResults(await searchProvider(provider, query, limit, env, searchOptions), query, limit);
      if (results.length > 0) {
        return {
          ok: true,
          strategy,
...
async function handleRequest(request, env) {
  ...
  if (["/search", "/fetch"].includes(url.pathname)) {
  ...
  if (url.pathname === "/fetch") {
    const result = await runFetch(body, env);
...
export const _test = { canonicalizeUrl, cleanText, parseDuckDuckGoResults, parseBingResults, configuredProviders, isTextContentType, normalizeFreshness, normalizeLanguage, runSearch, runFetch, safeFetch, handleRequest };
